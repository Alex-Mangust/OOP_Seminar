...
public class GroupeStudents_G implements Iterable<Student_G>, Comparable<GroupeStudents_G> {
     ...
     */
    public GroupeStudents_G(int numberGroupe, List<Person_G<Student_G>> students) {
        this.numberGroupe = numberGroupe;
        this.students = students.stream().map(Person_G::getPerson).collect(Collectors.toList());
    }
    ...
    public List<Student_G> getGroupe() {
        return students;
    }

    /** Метод задает список студентов */
    public void setStudents(List<Student_G> students) {
    ...
    public String toString() {
        ...
        for (Student_G student : students) {
    ...
    @Override
    public Iterator<Student_G> iterator() {
        return new StudentIerator_G(students);
    }

    /** Переопределенный метод, необходимый для сравнения экземпляров класса GroupeStudent */
    @Override
    public int compareTo(GroupeStudents_G o) {
